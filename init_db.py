...
def initialize_database():
    ...
    if os.path.exists(DB_PATH):
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('employees', 'projects', 'employee_projects')")
        ...
        if tables:
            ...
            if has_data:
                logger.info("Database already exists with data. Skipping initialization.")
                conn.close()
                return
